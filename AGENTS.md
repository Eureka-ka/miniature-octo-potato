...
- AI 痕迹检测：8 个离线统计特征（句长/段落均匀度、套话密度、连接词密度、低具体性、重复率、逻辑异常度/术语偏离度（相对附件1 30篇参照均值偏离）），权重 0.15/0.12/0.15/0.08/0.15/0.12/0.11/0.12；分档 <0.40 低、0.40~0.55 中、≥0.55 高。
...
- 优化后得分预测：提升对应指标 s 后重跑评分模型；当前预测 62.06 / 66.02 / 61.82（3-2 跨入良好）。
