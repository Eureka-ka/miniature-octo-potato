...
├─ quantify.py          # 问题1：16个二级指标专属量化 + 代码页检测（jieba可选）
...
- 分级：**相对分位定级** `percentile_bands`（85/65/40/20 分位；本次阈值：优秀≥72.8、良好≥69.8、中等≥65.7、及格≥63.1，以下为不及格）。
- 当前结果（29 篇自动评分）：**优秀5 / 良好5 / 中等7 / 及格6 / 不及格6**（得分区间 58.1~77.1）；25.pdf 标记图像型排除。
...
- 分析：灰色关联度(GRA)、Pearson/Spearman+Bootstrap95%CI+置换检验、关键特征识别、**预测模型（岭回归λ=1.0，单特征=内容字符数；样本内R²≈0.19、LOO-CV R²≈−0.47）**、Bootstrap(1000次)、单样本剔除敏感性。
- 质量调整因子：k=1+λ·(F−F̄)/F̄（λ=0.30），基于特征剖面相对位置校准基础得分，不依赖回归外推能力。
- 关键结论：可量化文本特征与质量得分的关联整体较弱且方向不一（内容字符数/页数弱正相关、连词/参考文献类弱负相关）；**正相关主要源于质量得分对篇幅的敏感性（长度偏置：公式总数 r≈0.18 但公式密度 r≈0.04）**；负相关且反直觉的特征不作预测输入；小样本（n=9）下模型定位为"关联识别与方向判断"，不宜直接打分。

### 问题3（problem3.py）
- 基线：3-1/3-2/3-3 = 59.79 / 62.56 / 59.04（均"中等"，与题目预设一致）；AI辅助指数 0.285/0.185/0.180（均"低"）。
- AI 痕迹检测：参考算法五维（句子重复度、句式变化性、段落均匀度、逻辑异常度/术语偏离度（相对附件1 30篇参照均值）），3篇内 min-max 后加权 0.30/0.25/0.20/0.15/0.10；分档 <0.30 低、0.30~0.70 中、≥0.70 高。
- 逻辑断层识别：规则扫描（公式化不足、缺假设、结论未验证、缺灵敏度/稳健性、摘要要素不全、参考文献不足、公式编号/符号说明缺失）。
- 修改方案：薄弱指标（s<0.6）+ 检测问题 → `TEMPLATE` 建议；增益 `uplift_for`（<0.3→+0.30、<0.5→+0.22、<0.7→+0.12、否则+0.05）；高AI论文对 AI 相关指标（u11/u13/u44）额外 +0.10。
- 优化后得分预测：提升对应指标 s 后重跑评分模型；当前预测 62.34 / 66.67 / 71.89（3-2、3-3 跨入良好）。
