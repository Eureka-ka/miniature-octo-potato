...
def main():
    ...
    para(doc, "本样本下各特征与质量得分的相关整体较弱且方向不一：|r|最大的为" + _top3_str + "等；篇幅类特征（总页数、总字符数）呈弱正相关，"
              ...
              "结合正相关方向与可解释性，选取 内容字符数（内容充实度，r≈0.43）作为预测模型自变量；"
              "连词密度、参考文献数量等呈负相关且符号与直觉相悖，不作为预测输入（其含义需结合小样本与篇幅效应解读）。")
    ...
    para(doc, f"以内容字符数（单特征，正相关）为自变量、质量得分为因变量建立标准化线性回归，并引入岭回归（λ=1.0）抑制共线性。"
    ...
    para(doc, "为兼顾数据视角与稳健性，定义质量调整因子：k_i=1+λ·(F_i−F̄)/F̄，其中λ=0.30为校准强度，"
              "F_i为论文特征剖面与理想剖面的灰色关联度。该因子基于论文特征剖面相对位置校准基础得分，不依赖特征回归的外推能力"
              "（当前LOO-CV R²<0，回归预测不可靠，故不进入调整因子）。调整后得分Q_adj=Q·k；"
              "调整后等级仅作参考、不覆盖基础等级，避免边界论文因±1~2分造成误判。")
    adj = p2s["质量调整因子"]
    ...
    para(doc, "AI检测采用参考算法的五维指标体系：句子重复度（句首8字重复占比）、句式变化性（1/(|句长变异系数−0.7|+0.3)）、段落均匀度（页块长度变异系数）、"
              "逻辑异常度（逻辑连接词密度偏离附件1 30篇均值）、术语偏离度（术语丰富度偏离附件1均值）。"
              "五个指标在3篇内min-max归一后按0.30/0.25/0.20/0.15/0.10加权合成AI辅助指数，映射为低/中/高三档（<0.30低、0.30~0.70中、≥0.70高）。"
              "该方法无需外部API，可完全复现；其局限在于仅为统计启发式，不能等同于大语言模型的困惑度检测，结果仅供参考。")
    ai = p3["AI痕迹检测"][["论文", "句子重复度", "句式变化性", "段落均匀度", "逻辑异常度", "术语偏离度",
                           "AI辅助指数", "辅助程度"]].copy()
    ...
    para(doc, "针对每篇论文得分低于0.6的薄弱指标及检测问题，给出按章节组织的具体修改建议（详见附录C与output/problem3_results.xlsx）。")
    ...
    para(doc, "按“薄弱指标得分越低、可改进空间越大”的原则设定增益（<0.30→+0.30，<0.50→+0.22，<0.70→+0.12，否则+0.05），"
              "对AI辅助程度为“高”的论文（如3-3）再对AI相关指标（逻辑连接、论证闭环、文本冗余）额外提升+0.10以体现“降低AI痕迹”的收益；"
