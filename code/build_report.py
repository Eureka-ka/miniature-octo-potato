...
def main():
    ...
    para(doc, f"30篇论文中，25.pdf为纯图片扫描件（无文字层），已标记为“图像型论文”并排除出自动评分；其余29篇自动识别完成。"
              f"分级结果为：{_p1dist}。在80/65/50/35阈值下，本批次得分集中于55.8~78.4，落在“中等—良好”区间："
              f"无论文达到80分优秀阈值，也无论文落入及格/不及格，反映该批次论文在结构与规范维度普遍达标、"
              f"但在逻辑闭环、创新、公式规范、参考文献等质量维度无明显拔尖。"
    ...
    para(doc, "本样本下各特征与质量得分的相关整体较弱且方向不一：|r|最大的为" + _top3_str + "等；篇幅类特征（总页数、总字符数）呈弱正相关，"
              "连词密度、规范编号公式占比等呈弱负相关，说明可量化文本特征对质量得分的线性解释力有限。"
